...
_HELP_BOX = {
    'en': '<div class="callout"><b>TWSStarter only launches TWS/Gateway — it does not keep them running.</b> It is a convenient tool to start and (when needed) stop TWS/Gateway and to show which are currently running. It does <b>not</b> start TWS automatically and does not restart it. To keep TWS/Gateway running throughout the trading week, use TWS\'s built-in <b>Auto Restart</b>: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — set the time and select <b>Auto restart</b>.</div>',
    'de': '<div class="callout"><b>TWSStarter startet TWS/Gateway nur — es hält sie nicht am Laufen.</b> Es ist ein komfortables Werkzeug, um TWS/Gateway zu starten und bei Bedarf zu stoppen und um anzuzeigen, welche gerade laufen. Es startet TWS <b>nicht</b> automatisch und startet es nicht neu. Damit TWS/Gateway die Handelswoche über durchlaufen, nutzen Sie die eingebaute <b>Auto Restart</b>-Funktion der TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — Zeit setzen und <b>Auto restart</b> wählen.</div>',
    'fr': '<div class="callout"><b>TWSStarter ne fait que lancer TWS/Gateway — il ne les maintient pas en marche.</b> C\'est un outil pratique pour démarrer et, au besoin, arrêter TWS/Gateway et pour voir lesquels sont en cours d\'exécution. Il ne démarre <b>pas</b> TWS automatiquement et ne le redémarre pas. Pour garder TWS/Gateway actif toute la semaine de bourse, utilisez la fonction <b>Auto Restart</b> intégrée de TWS : <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — réglez l\'heure et sélectionnez <b>Auto restart</b>.</div>',
    'es': '<div class="callout"><b>TWSStarter solo inicia TWS/Gateway — no los mantiene en ejecución.</b> Es una herramienta cómoda para iniciar y, si hace falta, detener TWS/Gateway y para mostrar cuáles están en ejecución. <b>No</b> inicia TWS automáticamente ni lo reinicia. Para mantener TWS/Gateway en ejecución durante la semana bursátil, use la función <b>Auto Restart</b> integrada de TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — fije la hora y seleccione <b>Auto restart</b>.</div>',
    'it': '<div class="callout"><b>TWSStarter avvia solo TWS/Gateway — non li mantiene in esecuzione.</b> È uno strumento comodo per avviare e, se serve, fermare TWS/Gateway e per mostrare quali sono in esecuzione. <b>Non</b> avvia TWS automaticamente e non lo riavvia. Per tenere TWS/Gateway attivo per tutta la settimana di borsa, usare la funzione <b>Auto Restart</b> integrata di TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — impostare l\'orario e selezionare <b>Auto restart</b>.</div>',
    'ru': '<div class="callout"><b>TWSStarter только запускает TWS/Gateway — он не поддерживает их работу.</b> Это удобный инструмент, чтобы запускать и при необходимости останавливать TWS/Gateway и показывать, какие из них сейчас работают. Он <b>не</b> запускает TWS автоматически и не перезапускает его. Чтобы TWS/Gateway работали в течение торговой недели, используйте встроенную функцию <b>Auto Restart</b> в TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — задайте время и выберите <b>Auto restart</b>.</div>',
    'nl': '<div class="callout"><b>TWSStarter start TWS/Gateway alleen — het houdt ze niet draaiend.</b> Het is een handig hulpmiddel om TWS/Gateway te starten en indien nodig te stoppen en om te tonen welke actief zijn. Het start TWS <b>niet</b> automatisch en herstart het niet. Om TWS/Gateway de hele handelsweek draaiend te houden, gebruikt u de ingebouwde <b>Auto Restart</b>-functie van TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — stel de tijd in en selecteer <b>Auto restart</b>.</div>',
    'pt': '<div class="callout"><b>O TWSStarter apenas inicia o TWS/Gateway — não os mantém em execução.</b> É uma ferramenta prática para iniciar e, se preciso, parar o TWS/Gateway e para mostrar quais estão em execução. <b>Não</b> inicia o TWS automaticamente nem o reinicia. Para manter o TWS/Gateway em execução durante a semana de negociação, use o recurso <b>Auto Restart</b> integrado do TWS: <i>Configuration → Lock and Exit → Auto Logoff Timer</i> — defina o horário e selecione <b>Auto restart</b>.</div>',
    'zh': '<div class="callout"><b>TWSStarter 只负责启动 TWS/Gateway——并不会让它们保持运行。</b> 它是一个方便的工具，用于启动、必要时停止 TWS/Gateway，并显示哪些正在运行。它<b>不会</b>自动启动 TWS，也不会重启它。若要让 TWS/Gateway 在整个交易周内持续运行，请使用 TWS 内置的 <b>Auto Restart</b> 功能：<i>Configuration → Lock and Exit → Auto Logoff Timer</i> —— 设置时间并选择 <b>Auto restart</b>。</div>',
    'ja': '<div class="callout"><b>TWSStarter は TWS/Gateway を起動するだけで、稼働させ続けはしません。</b> TWS/Gateway を起動し、必要に応じて停止し、どれが稼働中かを表示する便利なツールです。TWS を自動起動<b>しません</b>し、再起動もしません。取引週の間 TWS/Gateway を動かし続けるには、TWS 内蔵の <b>Auto Restart</b> 機能を使用してください：<i>Configuration → Lock and Exit → Auto Logoff Timer</i> —— 時刻を設定して <b>Auto restart</b> を選択します。</div>',
}
