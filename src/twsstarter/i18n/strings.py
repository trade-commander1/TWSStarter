...
_AUTOSTART_STRINGS = {
    'en': {'card_autostart': 'Autostart',      'tip_autostart': 'When on, TWSStarter keeps this connection running: it (re)starts it whenever it is not running.', 'msg_autostart': "Autostart: starting '{name}' …", 'section_autostart': 'Autostart', 'lbl_check_interval': 'Check interval', 'tip_check_interval': 'How often to check whether autostart connections are running and (re)start them. Also the delay before the first check after launch.'},
    'de': {'card_autostart': 'Autostart',      'tip_autostart': 'Wenn an, hält TWSStarter diese Verbindung am Laufen: sie wird (neu) gestartet, sobald sie nicht läuft.', 'msg_autostart': "Autostart: '{name}' wird gestartet …", 'section_autostart': 'Autostart', 'lbl_check_interval': 'Prüfintervall', 'tip_check_interval': 'Wie oft geprüft wird, ob Autostart-Verbindungen laufen, und sie (neu) gestartet werden. Auch die Verzögerung bis zur ersten Prüfung nach dem Start.'},
    'fr': {'card_autostart': 'Démarrage auto', 'tip_autostart': "Si activé, TWSStarter maintient cette connexion en marche : il la (re)démarre dès qu'elle n'est pas en cours.", 'msg_autostart': "Démarrage auto : '{name}' …", 'section_autostart': 'Démarrage auto', 'lbl_check_interval': 'Intervalle de vérification', 'tip_check_interval': "Fréquence de vérification que les connexions Autostart sont en cours et de leur (re)démarrage. Également le délai avant la première vérification après le lancement."},
    'es': {'card_autostart': 'Inicio auto',    'tip_autostart': 'Si está activado, TWSStarter mantiene esta conexión en ejecución: la (re)inicia siempre que no esté en ejecución.', 'msg_autostart': "Inicio auto: iniciando '{name}' …", 'section_autostart': 'Inicio auto', 'lbl_check_interval': 'Intervalo de comprobación', 'tip_check_interval': 'Con qué frecuencia se comprueba si las conexiones de Autostart están en ejecución y se (re)inician. También el retardo antes de la primera comprobación tras el arranque.'},
    'it': {'card_autostart': 'Avvio auto',     'tip_autostart': 'Se attivo, TWSStarter mantiene questa connessione in esecuzione: la (ri)avvia ogni volta che non è in esecuzione.', 'msg_autostart': "Avvio auto: avvio di '{name}' …", 'section_autostart': 'Avvio auto', 'lbl_check_interval': 'Intervallo di controllo', 'tip_check_interval': "Con quale frequenza controllare se le connessioni Autostart sono in esecuzione e (ri)avviarle. Anche il ritardo prima del primo controllo dopo l'avvio."},
    'ru': {'card_autostart': 'Автозапуск',     'tip_autostart': 'Если включено, TWSStarter поддерживает это соединение в работе: (пере)запускает его, как только оно не работает.', 'msg_autostart': "Автозапуск: запуск '{name}' …", 'section_autostart': 'Автозапуск', 'lbl_check_interval': 'Интервал проверки', 'tip_check_interval': 'Как часто проверять, работают ли соединения с автозапуском, и (пере)запускать их. Также задержка до первой проверки после старта.'},
    'nl': {'card_autostart': 'Autostart',      'tip_autostart': 'Indien aan, houdt TWSStarter deze verbinding draaiend: het (her)start ze zodra ze niet draait.', 'msg_autostart': "Autostart: '{name}' wordt gestart …", 'section_autostart': 'Autostart', 'lbl_check_interval': 'Controle-interval', 'tip_check_interval': 'Hoe vaak wordt gecontroleerd of Autostart-verbindingen draaien en worden ze (her)start. Ook de vertraging vóór de eerste controle na het opstarten.'},
    'pt': {'card_autostart': 'Início auto',    'tip_autostart': 'Se ativado, o TWSStarter mantém esta conexão em execução: (re)inicia-a sempre que não estiver em execução.', 'msg_autostart': "Início auto: iniciando '{name}' …", 'section_autostart': 'Início auto', 'lbl_check_interval': 'Intervalo de verificação', 'tip_check_interval': 'Com que frequência verificar se as conexões de Autostart estão em execução e (re)iniciá-las. Também o atraso antes da primeira verificação após a inicialização.'},
    'zh': {'card_autostart': '自动启动',        'tip_autostart': '开启后，TWSStarter 会让该连接保持运行：只要它未运行就会（重新）启动它。', 'msg_autostart': "自动启动：正在启动 '{name}' …", 'section_autostart': '自动启动', 'lbl_check_interval': '检查间隔', 'tip_check_interval': '多久检查一次 Autostart 连接是否在运行并（重新）启动它们。也是启动后首次检查前的延迟。'},
    'ja': {'card_autostart': '自動起動',        'tip_autostart': 'オンの場合、TWSStarter はこの接続を稼働し続けます。実行されていなければ（再）起動します。', 'msg_autostart': "自動起動：'{name}' を起動しています …", 'section_autostart': '自動起動', 'lbl_check_interval': '確認間隔', 'tip_check_interval': 'Autostart 接続が実行中かを確認し、（再）起動する頻度。起動後の初回確認までの遅延でもあります。'},
}
...
_HELP_BOX = {
    'en': '<div class="callout"><b>TWSStarter starts your TWS/Gateway connections and shows their status.</b> With <b>Autostart</b> on (per card) it also keeps a connection running: it checks periodically (see <b>Settings → Check interval</b>) and (re)starts it in its default mode whenever it is not running. Alternatively, for an in-place restart at TWS\'s daily reset, use TWS\'s own <b>Auto Restart</b> (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'de': '<div class="callout"><b>TWSStarter startet Ihre TWS/Gateway-Verbindungen und zeigt deren Status.</b> Mit <b>Autostart</b> (pro Karte) hält es eine Verbindung auch am Laufen: Es prüft regelmäßig (siehe <b>Einstellungen → Prüfintervall</b>) und startet sie im Standardmodus (neu), sobald sie nicht läuft. Alternativ nutzen Sie für einen In-Place-Neustart beim täglichen TWS-Reset die eingebaute <b>Auto Restart</b>-Funktion der TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'fr': '<div class="callout"><b>TWSStarter démarre vos connexions TWS/Gateway et affiche leur état.</b> Avec <b>Autostart</b> (par carte), il maintient aussi une connexion en marche : il vérifie régulièrement (voir <b>Paramètres → Intervalle de vérification</b>) et la (re)démarre dans son mode par défaut dès qu\'elle n\'est pas en cours. Sinon, pour un redémarrage sur place lors de la réinitialisation quotidienne de TWS, utilisez la fonction <b>Auto Restart</b> intégrée de TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'es': '<div class="callout"><b>TWSStarter inicia sus conexiones TWS/Gateway y muestra su estado.</b> Con <b>Autostart</b> (por tarjeta) también mantiene una conexión en ejecución: comprueba periódicamente (véase <b>Configuración → Intervalo de comprobación</b>) y la (re)inicia en su modo predeterminado siempre que no esté en ejecución. Como alternativa, para un reinicio in situ en el reinicio diario de TWS, use la función <b>Auto Restart</b> integrada de TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'it': '<div class="callout"><b>TWSStarter avvia le tue connessioni TWS/Gateway e ne mostra lo stato.</b> Con <b>Autostart</b> (per scheda) mantiene anche una connessione in esecuzione: controlla periodicamente (vedi <b>Impostazioni → Intervallo di controllo</b>) e la (ri)avvia nella sua modalità predefinita ogni volta che non è in esecuzione. In alternativa, per un riavvio sul posto al reset giornaliero di TWS, usa la funzione <b>Auto Restart</b> integrata di TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'ru': '<div class="callout"><b>TWSStarter запускает ваши соединения TWS/Gateway и показывает их статус.</b> С <b>Autostart</b> (на каждой карточке) он также поддерживает соединение в работе: периодически проверяет (см. <b>Настройки → Интервал проверки</b>) и (пере)запускает его в режиме по умолчанию, когда оно не работает. Как вариант, для перезапуска на месте при ежедневном сбросе TWS используйте встроенную функцию <b>Auto Restart</b> в TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'nl': '<div class="callout"><b>TWSStarter start uw TWS/Gateway-verbindingen en toont hun status.</b> Met <b>Autostart</b> (per kaart) houdt het een verbinding ook draaiend: het controleert periodiek (zie <b>Instellingen → Controle-interval</b>) en (her)start ze in haar standaardmodus zodra ze niet draait. Als alternatief gebruikt u voor een herstart ter plaatse bij de dagelijkse TWS-reset de ingebouwde <b>Auto Restart</b>-functie van TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'pt': '<div class="callout"><b>O TWSStarter inicia suas conexões TWS/Gateway e mostra o status.</b> Com <b>Autostart</b> (por cartão) também mantém uma conexão em execução: verifica periodicamente (veja <b>Configurações → Intervalo de verificação</b>) e a (re)inicia no seu modo padrão sempre que não estiver em execução. Como alternativa, para um reinício no local no reset diário do TWS, use o recurso <b>Auto Restart</b> integrado do TWS (<i>Configuration → Lock and Exit → Auto Logoff Timer</i>).</div>',
    'zh': '<div class="callout"><b>TWSStarter 启动你的 TWS/Gateway 连接并显示其状态。</b> 开启 <b>Autostart</b>（每张卡片）后，它还会让连接保持运行：定期检查（见<b>设置 → 检查间隔</b>），只要连接未运行就以其默认模式（重新）启动它。或者，若想在 TWS 每日重置时原地重启，可使用 TWS 内置的 <b>Auto Restart</b> 功能（<i>Configuration → Lock and Exit → Auto Logoff Timer</i>）。</div>',
    'ja': '<div class="callout"><b>TWSStarter は TWS/Gateway 接続を起動し、状態を表示します。</b> <b>Autostart</b>（各カード）をオンにすると、接続を稼働し続けます。定期的に確認し（<b>設定 → 確認間隔</b>参照）、実行されていなければ既定のモードで（再）起動します。あるいは、TWS の毎日のリセット時にその場で再起動するには、TWS 内蔵の <b>Auto Restart</b> 機能を使用してください（<i>Configuration → Lock and Exit → Auto Logoff Timer</i>）。</div>',
}

# 1b. Autostart paragraph, appended to the "Starting & Stopping" section.
_HELP_AUTOSTART = {
    'en': '<p>Each card has an <b>Autostart</b> switch (on by default). While it is on, TWSStarter checks every few seconds and starts the connection in its default mode whenever it is not running — so it comes back if you close it or it crashes. Set the interval (and the delay before the first check) under <b>Settings → Check interval</b> (default 30 s).</p>',
    'de': '<p>Jede Karte hat einen <b>Autostart</b>-Schalter (standardmäßig an). Solange er an ist, prüft TWSStarter alle paar Sekunden und startet die Verbindung im Standardmodus, sobald sie nicht läuft — sie kommt also zurück, wenn Sie sie schließen oder sie abstürzt. Das Intervall (und die Verzögerung bis zur ersten Prüfung) stellen Sie unter <b>Einstellungen → Prüfintervall</b> ein (Standard 30 s).</p>',
    'fr': '<p>Chaque carte possède un commutateur <b>Autostart</b> (activé par défaut). Tant qu\'il est activé, TWSStarter vérifie toutes les quelques secondes et démarre la connexion dans son mode par défaut dès qu\'elle n\'est pas en cours — elle revient donc si vous la fermez ou si elle plante. Réglez l\'intervalle (et le délai avant la première vérification) dans <b>Paramètres → Intervalle de vérification</b> (par défaut 30 s).</p>',
    'es': '<p>Cada tarjeta tiene un interruptor <b>Autostart</b> (activado por defecto). Mientras esté activado, TWSStarter comprueba cada pocos segundos e inicia la conexión en su modo predeterminado siempre que no esté en ejecución — así vuelve si la cierra o falla. Ajuste el intervalo (y el retardo antes de la primera comprobación) en <b>Configuración → Intervalo de comprobación</b> (predeterminado 30 s).</p>',
    'it': '<p>Ogni scheda ha un interruttore <b>Autostart</b> (attivo per impostazione predefinita). Finché è attivo, TWSStarter controlla ogni pochi secondi e avvia la connessione nella sua modalità predefinita ogni volta che non è in esecuzione — quindi ritorna se la chiudi o va in crash. Imposta l\'intervallo (e il ritardo prima del primo controllo) in <b>Impostazioni → Intervallo di controllo</b> (predefinito 30 s).</p>',
    'ru': '<p>На каждой карточке есть переключатель <b>Autostart</b> (по умолчанию включён). Пока он включён, TWSStarter каждые несколько секунд проверяет и запускает соединение в режиме по умолчанию, когда оно не работает — поэтому оно вернётся, если вы его закроете или оно упадёт. Задайте интервал (и задержку до первой проверки) в <b>Настройки → Интервал проверки</b> (по умолчанию 30 с).</p>',
    'nl': '<p>Elke kaart heeft een <b>Autostart</b>-schakelaar (standaard aan). Zolang die aan staat, controleert TWSStarter om de paar seconden en start de verbinding in haar standaardmodus zodra ze niet draait — ze komt dus terug als u ze sluit of ze crasht. Stel het interval (en de vertraging vóór de eerste controle) in via <b>Instellingen → Controle-interval</b> (standaard 30 s).</p>',
    'pt': '<p>Cada cartão tem um botão <b>Autostart</b> (ativado por padrão). Enquanto estiver ativado, o TWSStarter verifica a cada poucos segundos e inicia a conexão no seu modo padrão sempre que não estiver em execução — então ela volta se você a fechar ou ela travar. Defina o intervalo (e o atraso antes da primeira verificação) em <b>Configurações → Intervalo de verificação</b> (padrão 30 s).</p>',
    'zh': '<p>每张卡片都有一个 <b>Autostart</b> 开关（默认开启）。只要开启，TWSStarter 每隔几秒检查一次，只要连接未运行就以其默认模式启动它——因此如果你关闭它或它崩溃，它会重新出现。在<b>设置 → 检查间隔</b>中设置间隔（以及启动后首次检查前的延迟），默认 30 秒。</p>',
    'ja': '<p>各カードには <b>Autostart</b> スイッチがあります（既定でオン）。オンの間、TWSStarter は数秒ごとに確認し、接続が実行されていなければ既定のモードで起動します — 閉じたりクラッシュしたりしても復帰します。間隔（および起動後の初回確認までの遅延）は <b>設定 → 確認間隔</b> で設定します（既定 30 秒）。</p>',
}
